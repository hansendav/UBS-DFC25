...
import utils
from utils import ce_per_class, retreive_xy_center
...
class DFC25_TRAIN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        coords = retreive_xy_center(os.path.join(self.dataset_path, 'post-event', self.dataset_list[idx] + '_post_disaster' + self.suffix))
        ...
        return img.transpose(2, 0, 1), mask, coords
...
class Trainer: 
    def __init__(self, args):
        ...
        self.cl_weights = torch.tensor(args.class_weights).float().to(self.device)

        self.cl_criterion = nn.CrossEntropyLoss(
            reduction='none',
            weight=self.cl_weights
        )
    ...
    def training(self): 
        ...
        for epoch in range(self.num_epochs): 
            ...
            for batch_idx, (inputs, targets, coords) in enumerate(pbar_train): 
                ...
                loss_cl_per_class_batch = ce_per_class(cl_loss, targets, 4)
                mean_cl_loss = cl_loss.mean()
                ...
                run_loss_per_class.append(loss_cl_per_class_batch)
            ...
            run_loss_per_class = np.array(run_loss_per_class).mean(axis=0)
            ...
            self.writer.add_scalar('closs/train_background', run_loss_per_class[0].item(), epoch)
            self.writer.add_scalar('closs/train_intact', run_loss_per_class[1].item(), epoch)
            self.writer.add_scalar('closs/train_damaged', run_loss_per_class[2].item(), epoch)
            self.writer.add_scalar('closs/train_destroyed', run_loss_per_class[3].item(), epoch)
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets, coords) in enumerate(pbar_val): 
                if batch_idx > 5: break  # Profile only a few iterations
...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_weights', type=list, default=[1.18, 7.46, 149.29, 95.22])
